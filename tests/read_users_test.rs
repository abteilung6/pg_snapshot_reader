...
#[tokio::test]
async fn writes_staged_cdc_update_events_as_clickhouse_versions() -> anyhow::Result<()> {
    let client = connect_to_postgres().await?;

    let table_name = unique_table_name();
    let clickhouse_table_name = format!("{}_cdc_update", table_name);

    let stage_dir =
        std::env::temp_dir().join(format!("{}_cdc_update_clickhouse_stage", table_name));
    ...
    let deleted_flags = fetch_clickhouse_query(
        ...
        &format!(
            "
            SELECT groupArray(_replication_deleted)
            FROM {}
            ",
            clickhouse_table_name
        ),
    ...
    assert!(deleted_flags.contains("0"));
...
#[tokio::test]
async fn latest_state_query_ignores_deleted_rows() -> anyhow::Result<()> {
    let client = connect_to_postgres().await?;

    let table_name = unique_table_name();
    let clickhouse_table_name = format!("{}_cdc_latest_delete", table_name);

    let stage_dir = std::env::temp_dir().join(format!("{}_cdc_latest_delete_stage", table_name));
